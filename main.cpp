...
int main() {
	...
	//For menu flag value
	bool isProgramActive = true;//Sentinel value that will stop program if it was requested
	PendingQueue* pendingTransactions = new PendingQueue(5); //Delay between the transaction existence in the pedning queue is 5 seconds
	...
	//Initializing upcoming user ( Prompting to retrieve the name from the user )
	string userName;
	cout << "Please enter the name: ";
	cin >> userName;

	//Inserting new account of our new upcoming user ( could be made after all users, but this is for hands on experience ). Balance would be initialized by depositing some money
	...
	//Dummy accounts for adding them to binarySearchTree (bankAccounts)
	...
	//This part showcases capability of pending queue on processing the transactions
	// 
	// //Enqueue some pending transactions (using direct enqueue for simplicity; use addTransaction if adding rate limit)
	pendingTransactions->enqueuePendingQueueTransaction(new PendingQueueTransaction("00000002", "Deposit", 500.0));
	pendingTransactions->enqueuePendingQueueTransaction(new PendingQueueTransaction("00000003", "Withdraw", 200.0));
	pendingTransactions->enqueuePendingQueueTransaction(new PendingQueueTransaction("00000004", "Deposit", 1000.0));
	pendingTransactions->enqueuePendingQueueTransaction(new PendingQueueTransaction("99999999", "Deposit", 100.0));  //Invalid account for error test. It means account will not be found for this transaction
